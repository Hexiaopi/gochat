...
func (user *User) CreateSession() (session Session, err error) {
	...
	stmtin, err := database.DB.Prepare(statement)
	if err != nil {
		return
	}
	defer stmtin.Close()
	...
	stmtout, err := database.DB.Prepare("select id, uuid, email, user_id, created_at from sessions where uuid = ?")
	if err != nil {
		return
	}
	defer stmtout.Close()
...
func (user *User) Session() (session Session, err error) {
	session = Session{}
	err = database.DB.QueryRow("SELECT id, uuid, email, user_id, created_at FROM sessions WHERE user_id = ?", user.Id).
...
func (user *User) Create() (err error) {
	...
	stmtout, err := database.DB.Prepare("select id, uuid, created_at from users where uuid = ?")
	if err != nil {
		return
	}
	defer stmtout.Close()
...
func Users() (users []User, err error) {
	rows, err := database.DB.Query("SELECT id, uuid, name, email, password, created_at FROM users")
	if err != nil {
		return
	}
	for rows.Next() {
...
func UserByEmail(email string) (user User, err error) {
	user = User{}
	err = database.DB.QueryRow("SELECT id, uuid, name, email, password, created_at FROM users WHERE email = ?", email).
...
func UserByUUID(uuid string) (user User, err error) {
	user = User{}
	err = database.DB.QueryRow("SELECT id, uuid, name, email, password, created_at FROM users WHERE uuid = ?", uuid).
		Scan(&user.Id, &user.Uuid, &user.Name, &user.Email, &user.Password, &user.CreatedAt)
	return
}
...
func (user *User) CreateThread(topic string) (conv Thread, err error) {
	...
	stmtout, err := database.DB.Prepare("select id, uuid, topic, user_id, created_at from threads where uuid = ?")
	if err != nil {
		return
	}
	defer stmtout.Close()
...
func (user *User) CreatePost(conv Thread, body string) (post Post, err error) {
	...
	stmtout, err := database.DB.Prepare("select id, uuid, body, user_id, thread_id, created_at from posts where uuid = ?")
	if err != nil {
		return
	}
	defer stmtout.Close()
